        ...
        [&] {
            ...
            for (int64_t index = 0; index < metadata.numel; ++index) {
                ...
                const float value = compose_blend_value<mode>(
                    static_cast<float>(img1_data[index]),
                    static_cast<float>(img2_data[index]),
                    alpha,
                    max_value);
...
void blend_cpu(
    ...
    switch (blend_mode) {
        case BlendMode::Linear:
            blend_cpu_impl<BlendMode::Linear>(
                img1, img2, mask, output, metadata, max_value);
            break;
        case BlendMode::Multiply:
            blend_cpu_impl<BlendMode::Multiply>(
                img1, img2, mask, output, metadata, max_value);
            break;
        case BlendMode::Screen:
            blend_cpu_impl<BlendMode::Screen>(
                img1, img2, mask, output, metadata, max_value);
            break;
        case BlendMode::Overlay:
            blend_cpu_impl<BlendMode::Overlay>(
                img1, img2, mask, output, metadata, max_value);
            break;
    }
}
